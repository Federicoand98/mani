...
func (t *EditFileTool) Execute(ctx context.Context, input map[string]any) (string, error) {
	raw, ok := input["path"]
	if !ok {
		return "", fmt.Errorf("edit: missing required input 'path'")
	}

	path, ok := raw.(string)
	if !ok {
		return "", fmt.Errorf("edit: input 'path' should be a string")
	}

	rawOld, ok := input["old_content"]
	if !ok {
		return "", fmt.Errorf("edit: missing required input 'old_content'")
	}

	oldContent, ok := rawOld.(string)
	if !ok {
		return "", fmt.Errorf("edit: input 'old_content' should be a string")
	}

	if oldContent == "" {
		return "", fmt.Errorf("edit: input 'old_content' cannot be empty")
	}

	rawNew, ok := input["new_content"]
	if !ok {
		return "", fmt.Errorf("edit: missing required input 'new_content'")
	}

	newContent, ok := rawNew.(string)
	if !ok {
		return "", fmt.Errorf("edit: input 'new_content' should be a string")
	}
	...
	if occurrences == 0 {
		return "", fmt.Errorf("edit: old content not found in file")
	}
	if occurrences > 1 {
		return "", fmt.Errorf("edit: old content is ambiguous; found %d occurrences", occurrences)
	}
	...
	if err != nil {
		return "", fmt.Errorf("edit: failed to stat file: %w", err)
	}
